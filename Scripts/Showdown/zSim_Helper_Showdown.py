...
class FootballSimulation():
    ...
    def __init__(self, conn_sim, set_year, week):
        ...
        self.data = pd.read_sql_query(f'''SELECT * FROM showdown_week{week}_year{set_year}''', conn_sim)

        # add salaries to the dataframe and set index to player
        salaries = pd.read_sql_query(f'''SELECT player, pos, salary
                                         FROM Showdown_Salaries
                                         WHERE year={set_year}
                                               AND league={week} ''', conn_sim)

        self.data = pd.merge(self.data, salaries, how='left', on=['player', 'pos'])
