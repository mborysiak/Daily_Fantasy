...
rec_mean_cols = ['spread_line', 'total_line', 'vegas_wp', 
                 ...
                 'shotgun', 'no_huddle', 'pass_attempt', 'qb_dropback', 'qb_scramble', 'first_down_pass',
                 'third_down_converted', 'qb_hit',
                 'td_prob', 'qb_epa', 'wp', 'wpa',
                 'cp', 'cpoe', 'air_epa', 'air_wpa',
                 'air_yards', 'comp_air_epa', 'comp_air_wpa', 'comp_yac_epa',
...
rec_max_cols = ['td_prob', 'qb_epa', 'wp', 'wpa',
              'cp', 'cpoe', 'air_epa', 'air_wpa',
             'air_yards', 'comp_air_epa', 'comp_air_wpa', 'comp_yac_epa',
             'comp_yac_wpa',  'ep', 'epa', 'xyac_epa',
             'xyac_fd', 'xyac_mean_yardage', 'xyac_median_yardage', 'xyac_success',
             'yac_epa', 'yac_wpa', 'yards_after_catch',
             'yards_gained', 'ydstogo']

gcols =  ['week', 'season', 'posteam', 'receiver_player_name']
rec_sum = get_agg_stats(data[data.play_type=='pass'], gcols, rec_sum_cols, 'sum', prefix='rec')
rec_mean = get_agg_stats(data[data.play_type=='pass'], gcols, rec_mean_cols, 'mean', prefix='rec')
rec_max = get_agg_stats(data[data.play_type=='pass'], gcols, rec_max_cols, 'max', prefix='rec')
rec = pd.merge(rec_sum, rec_mean, on=gcols)
rec = pd.merge(rec, rec_max, on=gcols)
...
rush_sum_cols = ['shotgun', 'no_huddle', 'rush_attempt', 'first_down',
            ...
            'ep', 'epa', 'touchdown', 'fumble', 'fumble_lost','yardline_100',
            'yards_gained', 'ydstogo']

rush_mean_cols = ['td_prob', 'wp', 'wpa', 'ep', 'epa', 'yardline_100',
                  'yards_gained', 'ydstogo']

rush_max_cols = ['td_prob', 'wp', 'wpa', 'ep', 'epa','yards_gained', 'ydstogo']

gcols =  ['week', 'season', 'posteam', 'rusher_player_name']
rush_sum = get_agg_stats(data[data.play_type=='rush'], gcols, rush_sum_cols, 'sum', prefix='rush')
rush_mean = get_agg_stats(data[data.play_type=='rush'], gcols, rush_mean_cols, 'mean', prefix='rush')
rush_max = get_agg_stats(data[data.play_type=='rush'], gcols, rush_max_cols, 'max', prefix='rush')

rush = pd.merge(rush_sum, rush_mean, on=gcols)
rush = pd.merge(rush, rush_max, on=gcols)
...
qb_max_cols = [
             ...
             'xyac_fd', 'xyac_mean_yardage', 'xyac_median_yardage', 'xyac_success',
             'yac_epa', 'yac_wpa', 'yards_after_catch',
             'yards_gained', 'ydstogo', 'drive_first_downs', 'drive_play_count',
             'drive_time_of_possession']
...
pass_max = get_agg_stats(data[data.sack==0], gcols, qb_max_cols, 'max', prefix='pass')

qb = pd.merge(pass_sum, pass_mean, on=gcols)
qb = pd.merge(qb, pass_max, on=gcols)
...
rush_team_max_cols = [f'rush_{c}_max' for c in rush_max_cols]

rec_team_sum_cols = [f'rec_{c}_sum' for c in rec_sum_cols]
rec_team_mean_cols = [f'rec_{c}_mean' for c in rec_mean_cols]
rec_team_max_cols = [f'rec_{c}_max' for c in rec_max_cols]
...
team_rush_max = get_agg_stats(rush_max, gcols, rush_team_max_cols, 'max', prefix='team', suffix='')
...
team_rec_max = get_agg_stats(rec_max, gcols, rec_team_max_cols, 'max', prefix='team', suffix='')

team = data[gcols].drop_duplicates()
for d in [team_rush_sum, team_rush_mean, team_rush_max, team_rec_sum, team_rec_mean, team_rec_max]:
...
def_max_cols = ['qb_epa', 'wp', 'wpa',
                 'cp', 'cpoe', 'air_epa', 'air_wpa',
                  'comp_air_epa', 'comp_air_wpa', 'comp_yac_epa',
                 'comp_yac_wpa',  'ep', 'epa', 'xyac_epa',
                 'xyac_fd', 'xyac_mean_yardage', 'xyac_median_yardage', 'xyac_success',
                 'yac_epa', 'yac_wpa', 'yards_after_catch',
                 'yards_gained', 'ydstogo', 'return_yards'
...
defense_max = get_agg_stats(defense, gcols, def_max_cols, 'max', prefix='def')
defense_min = get_agg_stats(defense, gcols, def_min_cols, 'min', prefix='def')
...
def_scoring = pd.merge(def_scoring, defense_max, on=['defteam', 'season', 'week'])
def_scoring = pd.merge(def_scoring, defense_min, on=['defteam', 'season', 'week'])
