...
def create_sd_max_metrics(df):
    ...
    cols = ['fantasyPoints', 'ProjPts', 'projected_points', 'etr_proj_ceiling',
            'ffa_points', 'ffa_ceiling',  'max_proj_points', 'avg_proj_points',
            'pred_fp_per_game', 'std_dev']
...
def projection_data(pos, year, pred_vers, reg_ens_vers, std_dev_type, full_model_rel_weight):
    ...
    if pos == 'Defense':
        ...
        proj1 = dm.read(f'''SELECT player, player team, week, year, projected_points, fantasyPoints, ProjPts,
                                  ffa_points, ffa_ceiling,  max_proj_points, avg_proj_points
                           FROM Defense_Data_Week{max_week}''', f'Model_Features_{max_year}')
        ...
        proj = dm.read(f'''SELECT player, team, defTeam, week, year, projected_points, fantasyPoints, ProjPts,
                                  ffa_points, ffa_ceiling, max_proj_points, avg_proj_points, etr_proj_ceiling
                           FROM Backfill_{pos}_Week{max_week}
                            ''', f'Model_Features_{max_year}')
    
    if year >= 2023:
        preds = get_predictions(pred_vers, reg_ens_vers, std_dev_type, full_model_rel_weight)
        proj = pd.merge(proj, preds, on=['player', 'week', 'year'])
...
def get_max_metrics(week, year, pred_vers, reg_ens_vers, std_dev_type, full_model_rel_weight):
    ...
    for pos in ['QB', 'RB', 'WR', 'TE', 'Defense']:
        ...
        proj = projection_data(pos, year,  pred_vers, reg_ens_vers, std_dev_type, full_model_rel_weight)
...
def drop_teams_cur_week(preds, set_week, set_year):
    ...
    _, current = get_max_metrics(set_week, set_year, pred_vers, reg_ens_vers, std_dev_type, full_model_rel_weight)
    preds = pd.merge(preds, current[['player', 'pos', 'max_metric']], on=['player', 'pos'])
...
import itertools
covar_type = 'team_points_trunc_avgproj'
...
for set_week, set_year in zip(set_weeks, set_years):
    ...
    for pred_vers, reg_ens_vers, std_dev_type, full_model_rel_weight in iter_cats:
        ...
        player_data, _ = get_max_metrics(set_week, set_year, pred_vers, reg_ens_vers, std_dev_type, full_model_rel_weight)
