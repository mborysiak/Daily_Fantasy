...
def add_proj(df, set_week, set_year):
    ...
    def_table = f'Defense_Data_Quick_Week{set_week}'

    def_proj = dm.read(f'''SELECT player, team, 'DST' as pos, week, year,
                                dk_salary, projected_points, avg_proj_points,
                                etr_proj_points, fpts_proj_points, yahoo_salary,
                                fp_rank, rankadj_fp_rank 
                        FROM {def_table}
        ''', f"Model_Features_{set_year}")

    proj = pd.DataFrame()
    for pos in ['QB', 'RB', 'WR', 'TE']:
        pos_table = f'Backfill_{pos}_Data_Quick_Week{set_week}'
        cur_proj = dm.read(f'''SELECT player, team, pos, week, year,
                                    dk_salary, projected_points, avg_proj_points,
                                    etr_proj_points, fpts_proj_points, yahoo_salary,
                                    fp_rank, rankadj_fp_rank
                            FROM {pos_table}
...
def feature_engineering(df):
    ...
    for c in ['projected_points', 'avg_proj_points', 'etr_proj_points', 'fpts_proj_points', 
    ...
    for c in ['fp_rank', 'rankadj_fp_rank']:
        try: df[c+'_times_sal'] = df[c] * df.dk_salary
        except: pass

    df = df.sort_values(by=['team', 'pos', 'year', 'week', 'avg_proj_points']).reset_index(drop=True)
    df['team_pos_week_order'] = df.groupby(['team', 'pos', 'week', 'year']).cumcount().values

    df = df.sort_values(by=['pos', 'year', 'week', 'avg_proj_points']).reset_index(drop=True)
    ...
    avg_per_sal = df.groupby(['pos', 'year', 'week']).agg(avg_pts_per_sal_pos=('avg_proj_points', 'mean'))
    df = pd.merge(df, avg_per_sal, on=['pos', 'year', 'week'])
    df['avg_pts_per_sal_diff'] = df.avg_proj_points_over_sal - df.avg_pts_per_sal_pos
...
def remove_duplicates(df):
    ...
    max_pts = df.groupby(['player', 'week', 'year']).agg({'avg_proj_points': 'max'}).reset_index()
    df = pd.merge(df, max_pts, on=['player', 'week', 'year', 'avg_proj_points'])
...
def pull_ownership_val(ownership_table, ownership_vers, db_name):
...
def save_current_week_pred(ownership_vers, set_week, set_year, include_dst=True):
    ...
    sim_values = add_pred_values(sim_values, 'Predicted_Million', pred_version, 
                                 million_ens_vers, set_week, set_year, ownership_vers)
...
# for set_week, set_year in zip([2], [2024]):
...
if run_owner_model:
    ...
    val_predict['set_week'] = set_week
    val_predict['set_year'] = set_year
    dm.delete_from_db('Validations', 'Predicted_Ownership_Validation', f"set_week={set_week} AND set_year={set_year} AND ownership_vers='{ownership_vers}'", create_backup=False)
...
for ownership_vers in ['standard_ln', 'mil_times_standard_ln', 'mil_div_standard_ln', 'mil_only']:
    ...
    pred_player_ownership = pull_ownership_val('Predicted_Ownership_Validation', 'standard_ln', 'Validations')
