...
def append_to_db(df, db_name='Weekly_Stats.sqlite3', table_name='NA', if_exist='append'):
    ...
    os.chdir('/Users/Mark/Documents/Github/Daily_Fantasy/Data/')
...
def copy_db(week=set_week, year=set_year, pre_post='pre'):
    db = sqlite3.connect('/Users/Mark/Documents/Github/Daily_Fantasy/Data/Weekly_Stats.sqlite3')
    path = '/Users/Mark/Documents/Github/Daily_Fantasy/Data/Backups/Week{}_{}_{}.sqlite3'.format(week, year, pre_post)
    db2 = sqlite3.connect(path)
    path = '/Users/Mark/OneDrive/FF/Daily/Backups/Week{}_{}_{}.sqlite3'.format(week, year, pre_post)
    ...
    for table_name in tables:
        ...
        table.to_sql(table_name, db3, index=False, if_exists = 'replace')
    cursor.close()
    db.close()
    db2.close()
    db3.close()
...
# +
# nf = pd.read_html('https://www.numberfire.com/nfl/daily-fantasy/daily-football-projections')

# nf = nf[3]
# nf.columns = ['player_info', 'nf_fantasy_pts', 'fd_cost', 'nf_value', 'nf_completion_att', 'nf_pass_yds', 
#               'nf_pass_tds', 'nf_int', 'nf_rush_att', 'nf_rush_yds', 'nf_rush_tds', 'nf_rec', 'nf_rec_yds',
#               'nf_rec_tds', 'targets']

# nf['p_info'] = nf.player_info.apply(lambda x: [i for i in x.split(' ') if 'Jr.' not in i])
# nf['p_info'] = nf.p_info.apply(lambda x: [i for i in x if 'III' not in i])
# nf['p_info'] = nf.p_info.apply(lambda x: [i for i in x if 'Laan' not in i])

# nf['player'] = nf.p_info.apply(lambda x: ' '.join(x[2:4]))
# nf['position'] = nf.p_info.apply(lambda x: ''.join(x[4]))
# #nf['matchup'] = nf.p_info.apply(lambda x: x[6])
# nf['injury'] = nf.p_info.apply(lambda x: x[8])
# nf.loc[nf.injury.isin(['Sun', 'Mon', 'Thurs']), 'injury'] = 'NotListed'
# nf['nf_completion'] = nf.nf_completion_att.apply(lambda x: x.split('/')[0]).astype('float')
# nf['nf_pass_att'] = nf.nf_completion_att.apply(lambda x: x.split('/')[1]).astype('float')
...
data = pd.concat([data, pd.DataFrame({'away_team': ['HOU'], 'home_team': ['KC']})], axis=0, sort=False).reset_index(drop=True)
...
# cities = pd.read_html('https://sites.google.com/site/32nflteamsabsoluteandrelative/')[1].iloc[2:, :3].dropna()
# cities.columns = ['city', 'latitude', 'longitude']
# cities.latitude = cities.latitude.apply(lambda x: float(x.split(' ')[0]))
# cities.longitude = cities.longitude.apply(lambda x: -float(x.split(' ')[0]))
# cities = pd.concat([cities, pd.DataFrame({'city': 'Los Angeles',
#                                  'latitude': 34.0141, 
#                                  'longitude': -118.2879}, index=[34])], axis=0).reset_index(drop=True)
# cities.loc[cities.city == 'New York Giants', 'city'] = 'New York'
# cities.loc[cities.city == 'Tampa', 'city'] = 'Tampa Bay'
# cities = cities[cities.city != 'New York Jets'].reset_index(drop=True)
#
# city_map = pd.DataFrame(city_map, index=range(0, 1)).T.rename(columns={0: 'team'})
# cities = pd.merge(cities, city_map, how='inner', left_on='city', right_index=True)
# cities = pd.concat([cities, pd.DataFrame(to_app)], axis=0)
# append_to_db(cities, table_name='City_Lat_Long', if_exist='replace')
...
# +
df = pd.read_csv('/Users/Mark/Documents/GitHub/Daily_Fantasy/Data/pff_matchups/pff_wr_cb/wr_cb_week{}.csv'.format(str(set_week)),
                 header=None)
...
df.loc[df.player == 'Marquez Valdes-scantling', 'player'] = 'Marquez Valdes-Scantling'
df.loc[df.player == 'Juju Smith-schuster', 'player'] = 'JuJu Smith-Schuster'
df = df.fillna(0)
df['routes'] = df['routes'].apply(lambda x: float(str(x).replace('-', '0')))
df['routes_against'] = df['routes_against'].apply(lambda x: float(str(x).replace('-', '0')))
...
df.pff_rank_wr = df.pff_rank_wr.apply(fix_pct)
...
df = df[df.player != 'Chester Rogers']
df['week'] = set_week
df['year'] = set_year
# -

append_to_db(df, table_name='PFF_WR_CB_Matchups', if_exist='append')

# # TE PFF

# +
df = pd.read_csv('/Users/Mark/Documents/GitHub/Daily_Fantasy/Data/pff_matchups/pff_te/te_week{}.csv'.format(str(set_week)),
                 header=None)
idx = []
for i, row in df.iterrows():
    if len(row.values[0]) > 7:
...
# +
df = pd.read_csv('/Users/Mark/Documents/GitHub/Daily_Fantasy/Data/pff_matchups/pff_oline_dline/oline_dline_week{}.csv'.format(str(set_week)),
                 header=None)
...
# library(data.table)
# library(jsonlite)
# df_air <- data.table(fromJSON('http://airyards.com/2019/weeks'))
...
# +
rb_cols = ['player', 'rush_att', 'rush_yds', 'yards_per_att', 'long_rush', 'plus_twenty_runs', 'rush_td', 
           'rec', 'tgts', 'rec_yds', 'yards_per_rec', 'rec_td', 'fumbles_lost', 'games', 'fantasy_pts', 
           'fantasy_ppg', 'own_pct']

qb_cols = ['player', 'pass_complete', 'pass_att', 'pct_complete', 'pass_yds', 'pass_yd_per_att', 'pass_td', 'pass_int',
        'sacks', 'rush_att', 'rush_yds', 'rush_td', 'games', 'fumbles_lost', 'fantasy_pts', 'fantasy_ppg', 'own_pct']

wr_cols = ['player', 'rec', 'tgts', 'rec_yds', 'yards_per_rec', 'long_rec', 'plus_twenty_rec', 'rec_td',
           'rush_att', 'rush_yds', 'rush_td', 'fumbles_lost', 'games', 'fantasy_pts', 'fantasy_ppg',
           'own_pct']

def_cols = ['player', 'sacks', 'int', 'fumble_rec', 'forced_fumbles', 'def_td', 'safety', 
            'special_teams_td', 'games', 'fantasy_pts', 'fantasy_ppg', 'own_pct']

k_cols = ['player', 'fg_made', 'fg_att', 'pct_made', 'long', 'one_nineteen', 'twenty_twentynine', 
...
for df in [qb_weekly, rb_weekly, wr_weekly, te_weekly, k_weekly]:
    ...
    df['player'] = df.player.apply(lambda x: ' '.join(x.split('(')[:-1]).rstrip(' '))
    df['week'] = set_week
    df['year'] = set_year
    df.own_pct = df.own_pct.fillna('0').apply(lambda x: float(x.replace('%', '')))
...
rush_rz = pd.read_html('https://www.pro-football-reference.com/years/2019/redzone-rushing.htm')
rec_rz = pd.read_html('https://www.pro-football-reference.com/years/2019/redzone-receiving.htm')
pass_rz = pd.read_html('https://www.pro-football-reference.com/years/2019/redzone-passing.htm')
...
# +
qb = pd.read_csv('/Users/Mark/Documents/GitHub/Daily_Fantasy/Data/pfr_adv_stats/qb_week{}.csv'.format(set_week))
qb = qb.drop('1D', axis=1)
rb = pd.read_csv('/Users/Mark/Documents/GitHub/Daily_Fantasy/Data/pfr_adv_stats/rb_week{}.csv'.format(set_week))
rec = pd.read_csv('/Users/Mark/Documents/GitHub/Daily_Fantasy/Data/pfr_adv_stats/wr_week{}.csv'.format(set_week))
...
rec_cols = ['rank', 'player', 'team', 'age', 'position', 'games', 'games_started', 'targets', 'rec', 
            'rec_yds', 'first_downs', 'yds_before_catch', 'yds_before_catch_per_rec', 'yards_after_catch', 
            'yards_after_catch_per_rec', 'broken_tackles', 'rec_per_broken', 'drops', 'drop_pct']
rec.columns = rec_cols
rec = rec.drop('position', axis=1).fillna(0)
...
#append_to_db(qb_adv, table_name='PFR_QB_Adv', if_exist=r_or_a)
...
# qbr = pd.read_html('http://www.espn.com/nfl/qbr')[0]
# qbr = pd.read_csv('/Users/Mark/Desktop/qbr.csv')
# qbr = qbr[['RANK', 'NAME', 'PAA', 'PASS', 'RUN', 'PEN', 'EPA', 'PLAYS', 'RAW', 'QBR']]
qbr.columns = ['rank', 'player', 'pts_added', 'pass_added', 'run_added', 'penalty_added',
